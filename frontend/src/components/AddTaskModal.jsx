...
const AddTaskModal = ({setShowAddTaskModal, fetchTodos}) => {
  ...
  return (
    <div className='flex flex-col gap-4 w-full max-w-[20rem] bg-white mx-4 rounded-md sm:rounded-lg h-fit shadow-md p-4 relative'>
        ...
        <input className=' w-full h-10 pl-2 shadow bg-gray-200 rounded-lg text-sm outline-none' type="text" name='todo' placeholder='Add task here....' value={textValue} required onChange={(e)=>{setTextValue(e.target.value);  setError('')}} autoFocus autoComplete='off' />
        {error && <span className='text-red-500 font-semibold tracking-tight text-xs'>{error}</span>}
...
<span className='font-semibold text-md -mb-3'>Status:</span>
  ...
  title='Close'
  aria-label='Close Add Task Window'
